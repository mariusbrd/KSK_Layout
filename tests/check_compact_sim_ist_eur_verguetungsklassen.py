...
Kompakt plus Simulation -> IST-Analyse -> EUR -> Verguetungsklassen

Ziel-Stichtag ist fest auf 31.12.2026 gesetzt.

Geprueft wird:
1. Die Stufenautomatik veraendert die EUR-Verteilung ueber Entgeltgruppe x Stufe.
2. Die Gesamt-EUR der Matrix entspricht der deduplizierten Gesamt-EUR des simulierten Bestands.
3. Die Gesamt-EUR je Entgeltgruppe sinkt durch die Stufenautomatik nicht.
"""
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> Salary Automation -> IST EUR -> Verguetungsklassen")
    ...
    off_tbl = compact.create_stacked_tariff_breakdown_table(off_chart_df, "Total_Cost_Year")
    on_tbl = compact.create_stacked_tariff_breakdown_table(on_chart_df, "Total_Cost_Year")
    ...
    decreasing_groups = totals[totals["delta"] < -1e-6].copy()
    ...
    _assert(len(changed_steps) > 0, "Stufenautomatik veraendert die EUR-Verguetungsklassen fuer 31.12.2026 nicht.")
    _assert(abs(overall_off - expected_off) < 1e-6, f"Matrix-EUR off ({overall_off}) != Gesamt-EUR off ({expected_off})")
    _assert(abs(overall_on - expected_on) < 1e-6, f"Matrix-EUR on ({overall_on}) != Gesamt-EUR on ({expected_on})")
    _assert(decreasing_groups.empty, f"Entgeltgruppen-EUR sinkt unzulaessig: {decreasing_groups.to_dict(orient='records')}")
    _assert(overall_on > overall_off, "Gesamt-EUR steigt trotz aktiver Stufenautomatik nicht an.")

    print(f"Geaenderte Zellen (Stufe) : {len(changed_steps)}")
    print(f"Gesamt-EUR off           : {overall_off:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
    print(f"Gesamt-EUR on            : {overall_on:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
    print(f"Delta Gesamt-EUR         : {(overall_on - overall_off):,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
    for entgeltgruppe, stufe, before, after in changed_steps[:20]:
        delta = after - before
        print(
            f"- {entgeltgruppe} {stufe}: "
            f"{before:,.2f} -> {after:,.2f} ({delta:+,.2f})".replace(",", "X").replace(".", ",").replace("X", ".")
        )

    print("OK: IST-Analyse > EUR > Verguetungsklassen reagiert fuer 31.12.2026 konsistent auf die Stufenautomatik.")
