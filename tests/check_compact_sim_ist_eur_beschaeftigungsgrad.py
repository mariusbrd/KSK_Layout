...
def main() -> None:
    ...
    breakdown_df = compact.create_breakdown_table(emp_df, degree_col, "Total_Cost_Year")
    ...
    expected_df = (
        unique_emp.groupby("expected_degree_cat", observed=True)["Total_Cost_Year"]
        .sum()
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, "Beschaeftigungsgrad-Breakdown summiert sich nicht auf Gesamt-EUR roh.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"EUR fuer Kategorie '{label}' stimmt nicht mit deduplizierter Mitarbeitersicht ueberein.")

    print("OK: IST-Analyse > EUR > Beschaeftigungsgrad ist fuer 31.12.2026 konsistent.")
