...
Kompakt plus Simulation -> IST-Analyse -> EUR -> Dauer im Unternehmen
...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, TENURE_COL, "Total_Cost_Year")
    ...
    expected_df = (
        future_unique.groupby(TENURE_COL, observed=True)["Total_Cost_Year"]
        .sum()
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, f"Dauer-im-Unternehmen-Breakdown summiert sich nicht auf Gesamt-EUR roh fuer {target_date:%d.%m.%Y}.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"EUR fuer Tenure-Kategorie '{label}' stimmt fuer {target_date:%d.%m.%Y} nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(missing_forecast == 0, f"Simulierte Zugaenge haben fuer {target_date:%d.%m.%Y} keine gueltige Unternehmenszugehoerigkeit in der EUR-Sicht.")
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST EUR -> Dauer im Unternehmen")
    print("")
    ...
    print("OK: EUR-Tenure-Checks fuer 10 zufaellige Zukunftsdaten erfolgreich.")
