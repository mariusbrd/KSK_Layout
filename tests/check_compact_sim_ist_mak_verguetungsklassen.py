...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST MAK -> Verguetungsklassen")
    ...
    _assert(
        len(changed_steps) > 0,
        "Stufenautomatik veraendert die MAK-Verguetungsklassen fuer 31.12.2026 nicht.",
    )
    _assert(
        total_delta_groups.empty,
        f"Entgeltgruppen-MAK veraendert sich unzulaessig: {total_delta_groups.to_dict(orient='records')}",
    )
    _assert(abs(overall_off - expected_off) < 1e-6, f"Matrix-MAK off ({overall_off}) != Gesamt-MAK off ({expected_off})")
    _assert(abs(overall_on - expected_on) < 1e-6, f"Matrix-MAK on ({overall_on}) != Gesamt-MAK on ({expected_on})")

    print(f"Geaenderte Zellen (Stufe) : {len(changed_steps)}")
    print(f"Gesamt-MAK off           : {overall_off:.4f}")
    print(f"Gesamt-MAK on            : {overall_on:.4f}")
    for entgeltgruppe, stufe, before, after in changed_steps[:20]:
        delta = after - before
        print(f"- {entgeltgruppe} {stufe}: {before:.4f} -> {after:.4f} ({delta:+.4f})")

    print("OK: IST-Analyse > MAK > Verguetungsklassen reagiert fuer 31.12.2026 konsistent auf die Stufenautomatik.")
