...
def _run_single_check(
    *,
    ...
    degree_col = _resolve_degree_column(future_prepared)
    value_col = next((c for c in ("MAK_Calculated", "mak", "MAK") if c in future_unique.columns), "FTE_assigned")

    breakdown_df = compact.create_breakdown_table(future_emp, degree_col, value_col)
    ...
    total_mak_raw = float(future_unique[value_col].fillna(0.0).sum())

    if "FTE_person" in future_unique.columns:
        future_unique["expected_degree_cat"] = compact.categorize_employment_degree(future_unique["FTE_person"])
    elif "BsGrd" in future_unique.columns:
        future_unique["expected_degree_cat"] = compact.categorize_employment_degree(future_unique["BsGrd"] / 100.0)
    else:
        future_unique["expected_degree_cat"] = "(unbekannt)"

    expected_missing = int(future_unique["expected_degree_cat"].isna().sum())
    expected_df = (
        future_unique.groupby("expected_degree_cat", observed=True)[value_col]
        .sum()
    ...
    _assert(expected_missing == 0, f"Mindestens ein Mitarbeitender hat fuer {target_date:%d.%m.%Y} keine Beschaeftigungsgrad-Kategorie.")
    _assert(abs(breakdown_total - total_mak_raw) < 1e-6, f"Beschaeftigungsgrad-Breakdown summiert sich nicht auf Gesamt-MAK roh fuer {target_date:%d.%m.%Y}.")
    _assert(breakdown_order == expected_order, f"Die Reihenfolge des Beschaeftigungsgrad-Charts ist fuer {target_date:%d.%m.%Y} ungueltig.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"MAK fuer Kategorie '{label}' stimmt fuer {target_date:%d.%m.%Y} nicht mit deduplizierter Mitarbeitersicht ueberein.")

    return {
        "target_date": target_date,
        "total_mak_raw": total_mak_raw,
...
def main() -> None:
    ...
    for idx, target_date in enumerate(target_dates, start=1):
        ...
        print(f"    Gesamt MAK  : {result['total_mak_raw']:.4f}")
        print(f"    Reihenfolge : {', '.join(result['breakdown_order'])}")
        print("")

    print("OK: MAK-Beschaeftigungsgrad-Checks fuer 4 zufaellige Zukunftsdaten erfolgreich.")
