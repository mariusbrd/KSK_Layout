...
def main() -> None:
    ...
    print(f"Seed           : {SEED}")
    print(f"Pruefpfad      : Simulation -> prepare_compact_data -> IST Koepfe -> Dauer im Unternehmen")
    print("")
    ...
    for idx, target_date in enumerate(random_dates, start=1):
        ...
        expected_counts = {str(row[tenure_col]): int(row["IST"]) for _, row in expected_counts.iterrows()}
        ...
        _assert(missing_forecast == 0, f"Forecast-Zugaenge haben fuer {target_date:%d.%m.%Y} keine gueltige Unternehmenszugehoerigkeit.")
        _assert(negative_tenure == 0, f"Negative Betriebszugehoerigkeit fuer {target_date:%d.%m.%Y} gefunden.")

    print("")
    print(f"OK: Dauer-im-Unternehmen-Checks fuer {N_DATES} zufaellige Zukunftsdaten erfolgreich.")
