...
Kompakt plus Simulation -> IST-Analyse -> EUR -> Beschaeftigungsgrad
...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, degree_col, "Total_Cost_Year")
    ...
    expected_df = (
        future_unique.groupby("expected_degree_cat", observed=True)["Total_Cost_Year"]
        .sum()
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, f"Beschaeftigungsgrad-Breakdown summiert sich nicht auf Gesamt-EUR roh fuer {target_date:%d.%m.%Y}.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"EUR fuer Kategorie '{label}' stimmt fuer {target_date:%d.%m.%Y} nicht mit deduplizierter Mitarbeitersicht ueberein.")
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST EUR -> Beschaeftigungsgrad")
    print("")
    ...
    print("OK: EUR-Beschaeftigungsgrad-Checks fuer 10 zufaellige Zukunftsdaten erfolgreich.")
