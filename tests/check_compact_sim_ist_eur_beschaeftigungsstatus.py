...
def main() -> None:
    ...
    breakdown_df = compact.create_breakdown_table(emp_df, status_col, "Total_Cost_Year")
    ...
    expected_df = (
        unique_emp.groupby(status_col, observed=True)["Total_Cost_Year"]
        .sum()
    ...
    forecast_status_values = (
        forecast_unique.groupby(status_col, observed=True)["Total_Cost_Year"]
        .sum()
        .reset_index(name="IST")
    )
    forecast_status_values = {
    ...
    print(f"- Forecast-Status EUR   : {forecast_status_values}")
    print("")
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, "Beschaeftigungsstatus-Breakdown summiert sich nicht auf Gesamt-EUR roh.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"EUR fuer Status '{label}' stimmt nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(missing_forecast == 0, "Simulierte Zugaenge haben keinen gueltigen Beschaeftigungsstatus in der EUR-Sicht.")

    print("OK: IST-Analyse > EUR > Beschaeftigungsstatus ist fuer 31.12.2026 konsistent.")
