...
def main() -> None:
    ...
    breakdown_df = compact.create_breakdown_table(emp_df, TENURE_COL, "Total_Cost_Year")
    ...
    expected_df = (
        unique_emp.groupby(TENURE_COL, observed=True)["Total_Cost_Year"]
        .sum()
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, "Dauer-im-Unternehmen-Breakdown summiert sich nicht auf Gesamt-EUR roh.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"EUR fuer Tenure-Kategorie '{label}' stimmt nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(missing_forecast == 0, "Simulierte Zugaenge haben keine gueltige Unternehmenszugehoerigkeit in der EUR-Sicht.")
    _assert(negative_tenure == 0, "Es gibt negative Betriebszugehoerigkeit im simulierten Bestand.")

    print("OK: IST-Analyse > EUR > Dauer im Unternehmen ist fuer 31.12.2026 konsistent.")
