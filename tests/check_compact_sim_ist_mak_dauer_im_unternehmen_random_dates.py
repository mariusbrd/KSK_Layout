...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, TENURE_COL, value_col)
    ...
    expected_df = (
        future_unique.groupby(TENURE_COL, observed=True)[value_col]
        .sum()
    ...
    missing_unknown_mask = (
        future_unique[TENURE_COL].isna() |
        future_unique[TENURE_COL].astype(str).str.strip().isin(["", "nan", "(unbekannt)"])
    )
    missing_forecast = int(
        future_unique.loc[missing_unknown_mask, "PersNr"].astype(str).str.startswith(FORECAST_PREFIXES).sum()
    )
    negative_tenure = int((pd.to_numeric(future_unique[TENURE_YEARS_COL], errors="coerce") < 0).fillna(False).sum())

    _assert(result.metadata.get("used_simulation") is True, f"Simulation wurde fuer {target_date:%d.%m.%Y} nicht aktiv ausgefuehrt.")
    _assert(abs(breakdown_total - total_mak_raw) < 1e-6, f"Dauer-im-Unternehmen-Breakdown summiert sich nicht auf Gesamt-MAK roh fuer {target_date:%d.%m.%Y}.")
    _assert(breakdown_order == expected_order, f"Die Reihenfolge des Tenure-Charts ist fuer {target_date:%d.%m.%Y} ungueltig.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"MAK fuer Tenure-Kategorie '{label}' stimmt fuer {target_date:%d.%m.%Y} nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(missing_forecast == 0, f"Simulierte Zugaenge haben fuer {target_date:%d.%m.%Y} keine gueltige Unternehmenszugehoerigkeit in der MAK-Sicht.")
    _assert(negative_tenure == 0, f"Es gibt negative Betriebszugehoerigkeit fuer {target_date:%d.%m.%Y}.")
...
def main() -> None:
    ...
    print("OK: MAK-Tenure-Checks fuer 4 zufaellige Zukunftsdaten erfolgreich.")
