
import pandas as pd
import sys
import os

# Add parent dir to path to import modules
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Mock streamlit session state
class MockSessionState(dict):
    def __getattr__(self, key):
        return self.get(key)
    def __setattr__(self, key, value):
        self[key] = value

# Mock streamlit
import streamlit as st
if not hasattr(st, "session_state"):
    st.session_state = MockSessionState()

def verify_filter_zoom():
    print("--- Verifying Page 4 Filter Logic (Zoom Fix) ---")
    
    # 1. Setup Mock Data
    snapshot_df = pd.DataFrame({
        "PersNr": ["1", "2", "3"],
        "OE-Cluster": ["Cluster A", "Cluster A", "Cluster B"],
        "Organisationseinheit": ["Org A", "Org A", "Org B"],
        "Jobfamily": ["JF1", "JF1", "JF2"]
    })
    
    # Mock Forecast Result (Events)
    # Event 1: New Hire in Cluster A (simulated via Org A)
    # Event 2: New Hire in Cluster B
    events_df = pd.DataFrame({
        "persnr": ["NH_1", "NH_2"],
        "Organisationseinheit": ["Org A", "Org B"],
        "OE-Cluster": ["Cluster A", "Cluster B"],
        "count": [1, 1]
    })
    
    print("\n[Input] Events Global:")
    print(events_df[["persnr", "Organisationseinheit", "OE-Cluster"]])
    
    # 2. Simulate Fix: Plural Keys + Robust Normalization
    print("\n... Simulating Filter: OE-Cluster = ['Cluster A'] ...")
    # THE FIX: Plural Key
    st.session_state["selected_oe_clusters"] = ["Cluster A"]
    
    # THE FIX: Robust Normalization helper
    def _apply_robust_filter(df, column, selected):
        if not selected or column not in df.columns:
            return df
        import re
        # Normalize data strings
        s_norm = df[column].astype(str).str.strip().str.replace(r"\.0$", "", regex=True)
        # Normalize filter strings
        vals_norm = [str(v).strip().replace(".0", "") for v in selected]
        return df[s_norm.isin(vals_norm)]

    events_view = events_df.copy()
    events_view = _apply_robust_filter(events_view, "OE-Cluster", st.session_state.get("selected_oe_clusters", []))
    
    print("\n[Output] Filtered Events (View):")
    print(events_view[["persnr", "Organisationseinheit", "OE-Cluster"]])
    
    # 3. Assertions
    if len(events_view) == 1 and events_view.iloc[0]["OE-Cluster"] == "Cluster A":
        print("\n[SUCCESS] Filter correctly zoomed into Cluster A using PLURAL key.")
    else:
        print("\n[FAILURE] Filter did not work as expected.")
        sys.exit(1)
        
    # 4. Normalization Test (123 vs 123.0)
    print("\n... Testing Normalization (Org B vs Org B.0) ...")
    st.session_state["selected_org_units"] = [123] # User chose Org Unit 123
    events_num = pd.DataFrame({"Organisationseinheit": ["123.0", "456"], "persnr": ["NH_X", "NH_Y"]})
    
    res = _apply_robust_filter(events_num, "Organisationseinheit", st.session_state["selected_org_units"])
    if len(res) == 1 and res.iloc[0]["persnr"] == "NH_X":
         print("[SUCCESS] Normalization handled 123 vs 123.0")
    else:
         print("[FAILURE] Normalization failed.")
         print(f"Result:\n{res}")
         sys.exit(1)

if __name__ == "__main__":
    verify_filter_zoom()
