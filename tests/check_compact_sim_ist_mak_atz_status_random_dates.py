...
def main() -> None:
    ...
    for idx, target_date in enumerate(random_dates, start=1):
        ...
        forecast_total = float(forecast_unique[value_col].fillna(0.0).sum())
        forecast_atz = (
        ...
        _assert(abs(float(breakdown_df['IST'].sum()) - total_mak_raw) < 1e-6, f"Breakdown summiert sich fuer {target_date:%d.%m.%Y} nicht auf Gesamt-MAK roh.")
    ...
    print("")
    print(f"OK: MAK-ATZ-Status-Checks fuer {N_DATES} zufaellige Zukunftsdaten erfolgreich.")
