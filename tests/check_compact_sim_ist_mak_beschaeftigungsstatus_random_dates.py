...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, status_col, value_col)
    ...
    expected_df = (
        future_unique.groupby(status_col, observed=True)[value_col]
        .sum()
    ...
    _assert(abs(breakdown_total - total_mak_raw) < 1e-6, f"Beschaeftigungsstatus-Breakdown summiert sich nicht auf Gesamt-MAK roh fuer {target_date:%d.%m.%Y}.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"MAK fuer Status '{label}' stimmt fuer {target_date:%d.%m.%Y} nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(missing_forecast == 0, f"Simulierte Zugaenge haben fuer {target_date:%d.%m.%Y} keinen gueltigen Beschaeftigungsstatus in der MAK-Sicht.")
...
def main() -> None:
    ...
    print("OK: MAK-Beschaeftigungsstatus-Checks fuer 4 zufaellige Zukunftsdaten erfolgreich.")
