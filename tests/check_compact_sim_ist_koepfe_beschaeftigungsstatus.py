...
Kompakt plus Simulation -> IST-Analyse -> Koepfe -> Beschaeftigungsstatus
...
4. Beschaeftigungsstatus-Breakdown ueber create_breakdown_table(..., "Headcount")
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST Koepfe -> Beschaeftigungsstatus")
    ...
    breakdown_df = compact.create_breakdown_table(emp_df, status_col, "Headcount")
    ...
    breakdown_counts = {
        str(row[status_col]): int(row["IST"])
        for _, row in breakdown_df.iterrows()
    }
    breakdown_total = int(breakdown_df["IST"].sum())

    expected_counts = (
        unique_emp.groupby(status_col, observed=True)["PersNr"]
        .nunique()
    ...
    forecast_status_counts = forecast_unique[status_col].astype(str).value_counts(dropna=False).to_dict()
    ...
    print(f"- Forecast-Status       : {forecast_status_counts}")
    print("")

    _assert(result.metadata.get("used_simulation") is True, "Simulation wurde fuer 31.12.2026 nicht aktiv ausgefuehrt.")
    _assert(breakdown_total == total_heads, "Beschaeftigungsstatus-Breakdown summiert sich nicht auf Gesamt-Koepfe.")
    _assert(breakdown_counts == expected_counts, "Chart-Breakdown stimmt nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(missing_forecast == 0, "Simulierte Zugaenge haben keinen gueltigen Beschaeftigungsstatus.")

    print("OK: IST-Analyse > Koepfe > Beschaeftigungsstatus ist fuer 31.12.2026 konsistent.")
