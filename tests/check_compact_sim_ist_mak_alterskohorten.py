...
Kompakt plus Simulation -> IST-Analyse -> MAK -> Alterskohorten

Ziel-Stichtag ist fest auf 31.12.2026 gesetzt.
Der Check verwendet denselben Pfad wie die Seite und prueft sowohl
die MAK-Verteilung als auch die Altersfortschreibung identischer Personen.
"""
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST MAK -> Alterskohorten")
    ...
    total_mak = float(compute_fte_effektiv(future_emp))
    total_mak_raw = float(future_unique[value_col].fillna(0.0).sum())
    breakdown_df = compact.create_breakdown_table(future_emp, "Alterskohorte", value_col)
    ...
    breakdown_values = {
        str(row["Alterskohorte"]): float(row["IST"])
    ...
    shared = base_unique.merge(
        future_unique[["PersNr", "Alter_Jahre", "Alterskohorte", value_col]],
        on="PersNr",
    ...
    print(f"- Kohorten gesamt       : {breakdown_values}")
    ...
    _assert(abs(breakdown_total - total_mak_raw) < 1e-6, "Alterskohorten-Breakdown summiert sich nicht auf Gesamt-MAK roh.")
    ...
    print("OK: IST-Analyse > MAK > Alterskohorten ist fuer 31.12.2026 konsistent.")
