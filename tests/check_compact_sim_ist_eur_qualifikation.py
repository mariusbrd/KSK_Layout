...
def main() -> None:
    ...
    breakdown_df = compact.create_breakdown_table(emp_df, "Ausbildung", "Total_Cost_Year")
    ...
    breakdown_values = {
        str(row["Ausbildung"]): float(row["IST"])
    ...
    ) if not edu_series.empty else len(unique_emp)
    ...
    _assert(missing_education == 0, "Es gibt Mitarbeitende ohne Ausbildung; die EUR-Qualifikationsdarstellung waere unvollstaendig.")
    _assert(abs(total_cost - total_cost_raw) < 1e-6, "Gesamt-EUR und deduplizierte Gesamt-EUR stimmen nicht ueberein.")
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, "Qualifikations-Breakdown summiert sich nicht auf Gesamt-EUR roh.")

    print("OK: IST-Analyse > EUR > Qualifikation ist fuer 31.12.2026 konsistent.")
