...
Kompakt plus Simulation -> IST-Analyse -> MAK -> Qualifikation
...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, "Ausbildung", value_col)
    ...
    edu = future_unique["Ausbildung"] if "Ausbildung" in future_unique.columns else pd.Series(dtype=object)
    missing_education = int(
        edu.isna().sum() +
        edu.astype(str).str.strip().eq("").sum() +
        edu.astype(str).str.strip().eq("nan").sum()
    ) if not edu.empty else len(future_unique)
    invalid_labels = sorted(
        {
            str(value).strip()
            for value in edu.dropna().astype(str)
    ...
    _assert("Ausbildung" in future_unique.columns, f"Spalte 'Ausbildung' fehlt fuer {target_date:%d.%m.%Y}.")
    _assert(missing_education == 0, f"Es gibt Mitarbeitende ohne Ausbildung fuer {target_date:%d.%m.%Y}.")
    _assert(not invalid_labels, f"Breakdown enthaelt ungueltige Qualifikationslabels fuer {target_date:%d.%m.%Y}: {invalid_labels}")
    _assert(abs(breakdown_total - total_mak_raw) < 1e-6, f"Qualifikations-Breakdown summiert sich nicht auf Gesamt-MAK roh fuer {target_date:%d.%m.%Y}.")
    _assert(breakdown_order == expected_order, f"Die Reihenfolge der Qualifikations-Labels stimmt fuer {target_date:%d.%m.%Y} nicht.")
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST MAK -> Qualifikation")
    print("")
    ...
    print("OK: MAK-Qualifikations-Checks fuer 4 zufaellige Zukunftsdaten erfolgreich.")
