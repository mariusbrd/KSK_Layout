...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, "Geschlecht", "Total_Cost_Year")
    ...
    breakdown_values = {str(row["Geschlecht"]): float(row["IST"]) for _, row in breakdown_df.iterrows()}
    breakdown_total = float(breakdown_df["IST"].sum())
    missing_gender = int(future_unique["Geschlecht"].isna().sum())
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, f"Geschlecht-Breakdown summiert sich fuer {target_date:%d.%m.%Y} nicht auf Gesamt-EUR roh.")
...
def main() -> None:
    ...
    print("OK: EUR-Geschlecht-Checks fuer 10 zufaellige Zukunftsdaten erfolgreich.")
