...
Kompakt plus Simulation -> IST-Analyse -> EUR -> Qualifikation
...
def _run_single_check(
    *,
    ...
    breakdown_df = compact.create_breakdown_table(future_emp, "Ausbildung", "Total_Cost_Year")
    ...
    edu_series = future_unique["Ausbildung"] if "Ausbildung" in future_unique.columns else pd.Series(dtype=object)
    missing_education = int(
        edu_series.isna().sum() +
        edu_series.astype(str).str.strip().eq("").sum() +
        edu_series.astype(str).str.strip().eq("nan").sum()
    ) if not edu_series.empty else len(future_unique)
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, f"Qualifikations-Breakdown summiert sich nicht auf Gesamt-EUR roh fuer {target_date:%d.%m.%Y}.")
...
def main() -> None:
    ...
    print("Pruefpfad      : Simulation -> prepare_compact_data -> IST EUR -> Qualifikation")
    print("")
    ...
    print("OK: EUR-Qualifikations-Checks fuer 10 zufaellige Zukunftsdaten erfolgreich.")
