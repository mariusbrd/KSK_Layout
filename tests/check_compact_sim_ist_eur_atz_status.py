...
def main() -> None:
    ...
    print(f"- Forecast-ATZ EUR      : {forecast_atz_values}")
    print("")
    ...
    _assert(abs(breakdown_total - total_cost_raw) < 1e-6, "ATZ-Status-Breakdown summiert sich nicht auf Gesamt-EUR roh.")
    ...
    for label, expected in expected_values.items():
        ...
        _assert(abs(actual - expected) < 1e-6, f"EUR fuer ATZ-Status '{label}' stimmt nicht mit deduplizierter Mitarbeitersicht ueberein.")
    _assert(invalid_count == 0, "Der simulierte Bestand enthaelt ungueltige ATZ-Statuswerte in der EUR-Sicht.")
    _assert(invalid_forecast == 0, "Forecast-Zugaenge haben ungueltige ATZ-Statuswerte in der EUR-Sicht.")
    _assert(abs(forecast_atz_values.get("Kein ATZ", 0.0) - float(forecast_unique["Total_Cost_Year"].fillna(0.0).sum())) < 1e-6, "Forecast-Zugaenge haben unerwartete ATZ-Statuswerte.")

    print("OK: IST-Analyse > EUR > ATZ-Status ist fuer 31.12.2026 konsistent.")
