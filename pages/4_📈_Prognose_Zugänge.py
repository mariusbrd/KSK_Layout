    ...
    if submit:
        ...
        run_params = {
            "azubi": {
                ...
                "jf_to_cluster_map": {str(k).strip(): v for k, v in zip(snapshot_df["Jobfamily"], snapshot_df["JF-Cluster"]) if v != "Unclustered" and str(v).strip() not in ("", "nan")} if "Jobfamily" in snapshot_df.columns and "JF-Cluster" in snapshot_df.columns else {}
            },
            "trainee": {
                "active": use_trainees,
        ...
        if not events_df.empty:
            ...
            if "JF-Cluster" in snapshot_df.columns:
                ...
                # Remove stale "Unclustered" values from snapshot map
                ...
                if "JF-Cluster" in events_df.columns:
                    # Preserve existing valid clusters, fill gaps from snapshot
                ...
                if mask_still_uncl.any() and "Jobfamily" in events_df.columns:
                    ...
                    # Apply to unclustered rows (enrich_jf_clusters now returns "Sonstiges" as fallback)
                    subset = events_df.loc[mask_still_uncl]
                    events_df.loc[mask_still_uncl, "JF-Cluster"] = enrich_jf_clusters(subset, {}, snap_jf_map)

                # c) Final safety-net: Replace any remaining "Unclustered" or NaN with "Sonstiges"
            ...
            # ── Fallback-Flag für Debug-Transparenz ──
            # Bestimme, welche Einträge ihren JF-Cluster nur über Fallback erhalten haben
            # (JF-Cluster == "Sonstiges" UND Jobfamily war in keiner Mapping-Quelle).
            _alias_keys = {"sonstige", "sonstiges", "trainee", "azubi"}
            _known_jf_keys = set(_alias_keys)  # Start with alias keys

            # Add keys from jf_to_cluster_map (passed as param)
            _jf_param_map = run_params.get("azubi", {}).get("jf_to_cluster_map", {})
            _known_jf_keys.update(str(k).strip().lower() for k in _jf_param_map.keys())

            # Add keys from cluster file
            try:
                from dataloader.cluster_manager import load_cluster_mappings as _lcm
                _, _file_jf_map = _lcm()
                for k in _file_jf_map.keys():
                    if isinstance(k, tuple):
                        pass  # Tuple keys are (Org, Planstelle), not Jobfamily names
                    else:
                        _known_jf_keys.add(str(k).strip().lower())
            except Exception:
                pass

            # Add keys from snapshot inheritance
            if "Jobfamily" in snapshot_df.columns and "JF-Cluster" in snapshot_df.columns:
                ...
                _known_jf_keys.update(str(k).strip().lower() for k in _snap_clean["Jobfamily"])

            # Build the flag
            # _jf_fallback = True means "real mapping gap" (not OrgUnit-mode deliberate Sonstiges)
            if "Jobfamily" in events_df.columns:
                ...
                # Exclude OrgUnit-mode events: they are deliberately "Sonstiges" (business rule)
                _is_orgunit_deliberate = events_df.get("_jf_orgunit_mode", pd.Series(False, index=events_df.index)).fillna(False).astype(bool)
        ...
        if st.session_state.get("chk_forecast_debug", False):
            with st.expander("📊 Debug: Diagnose nach Simulation", expanded=True):
                # ── Gleiche Datenbasis wie JF-Charts ──
                ...
                # ── Unclustered-Check (Pflicht: muss 0 sein) ──
                ...
                uncl_total = (_chart_basis["JF-Cluster"] == "Unclustered").sum()
                if uncl_total == 0:
                ...
                # Three categories within JF-Cluster == "Sonstiges":
                # 1. Fallback (echtes Mapping-Defizit) → _jf_fallback == True
                # 2. OrgUnit-Modus (fachlich korrekt, kein JF mapping erwartet) → _jf_orgunit_mode == True
                # 3. Explizit gemappt (Alias: Sonstige/Trainee/Azubi) → Rest
                ...
                if _has_flag:
                    ...
                    _jf_low = _chart_basis["Jobfamily"].astype(str).str.strip().str.lower() if "Jobfamily" in _chart_basis.columns else pd.Series("", index=_chart_basis.index)
                    _fb_mask = (_chart_basis["JF-Cluster"] == "Sonstiges") & (~_jf_low.isin(_alias_jfs))

                # OrgUnit-mode entries (deliberate business rule)
                _orgunit_mask = _chart_basis.get("_jf_orgunit_mode", pd.Series(False, index=_chart_basis.index)).fillna(False).astype(bool)
                ...
                _sonstiges_total = (_chart_basis["JF-Cluster"] == "Sonstiges").sum()
                ...
                col_s4.metric("Anteil Fallback an Chart-Basis", f"{(_fb_count / _total_chart * 100):.1f} %" if _total_chart > 0 else "n/a")
                ...
                if not _fb_rows.empty and "Jobfamily" in _fb_rows.columns:
                    ...
                    # Top Eventtyp pro Jobfamily
                    _fb_jf_types = _fb_rows.groupby("Jobfamily")["type"].apply(
                ...
                # ── D) Beispielzeilen zur Plausibilisierung ──
                st.divider()
                st.markdown("#### D) Beispielzeilen (Fallback)")

                if not _fb_rows.empty:
                    _example_cols = ["date", "type", "Jobfamily", "Organisationseinheit", "Planstelle", "JF-Cluster", "OE-Cluster", "mak"]
                    _example_cols = [c for c in _example_cols if c in _fb_rows.columns]

                    # Top 3 Eventtypen mit Fallback
                ...
                if not _fb_rows.empty and "Jobfamily" in _fb_rows.columns:
                    ...
                    # Vergleich mit Mapping-Quellen
                    st.markdown("**Jobfamilies im Fallback vs. aktive Mapping-Quellen:**")

                    # Lade aktive Mappings für Diagnose
                    try:
                        from dataloader.cluster_manager import load_cluster_mappings as _lcm_diag
                        _, _diag_jf_map = _lcm_diag()
                        _diag_keys_raw = []
                        for k in _diag_jf_map.keys():
                            if isinstance(k, tuple):
                                _diag_keys_raw.append(f"({k[0]}, {k[1]})")
                            else:
                                _diag_keys_raw.append(str(k).strip())
                        _diag_keys_lower = {str(k).strip().lower() for k in _diag_jf_map.keys() if not isinstance(k, tuple)}
                        ...
                        for jf in _fb_jf_unique:
                            ...
                            # Check for near-matches (Whitespace/Case issues)
                            near = [k for k in _diag_keys_lower if k.replace(" ", "") == jf_l.replace(" ", "") and k != jf_l]
                        ...
                        _missing_count = sum(1 for r in _diag_rows if r["In Cluster-Datei"] == "NEIN" and r["In Snapshot-Map"] == "NEIN")
                        if _missing_count > 0:
                            st.warning(
                            ...
                            st.info("Alle Fallback-Jobfamilies sind in mindestens einer Quelle vorhanden (evtl. Format-Abweichung).")

                        # Mapping-Typ Info
                        if _diag_jf_map:
                            _first_k = next(iter(_diag_jf_map.keys()))
                            _map_type = "Tupel-basiert (Org, Planstelle)" if isinstance(_first_k, tuple) else "String-basiert (Jobfamily)"
                            st.caption(f"Cluster-Datei Mapping-Typ: **{_map_type}** | Einträge: **{len(_diag_jf_map)}**")
                            if isinstance(_first_k, tuple):
                                st.caption(
                                    "Bei Tupel-basiertem Mapping wird nach (Organisationseinheit, Planstelle) gesucht, "
                                    "nicht nach Jobfamily-Namen. Deshalb können Jobfamilies, die als String vorhanden sind, "
                                    "trotzdem nicht gemappt werden."
                                )
                        else:
                ...
                st.divider()
                st.caption(
                    "Hinweis: 'Fallback' = Jobfamily gesetzt, aber kein Mapping vorhanden → JF-Cluster wird 'Sonstiges'. "
                    "'Explizit gemappt' = Sonstige/Trainee/Azubi via Alias-Regel → fachlich korrekt."
                )
