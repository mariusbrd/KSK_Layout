...
def _get_settings_tab_labels() -> list[str]:
    return [
        t("hiring.settings.tab.azubi"),
        t("hiring.settings.tab.trainee"),
        t("hiring.settings.tab.hires"),
    ]


def _get_result_tab_labels() -> list[str]:
    return [
        t("hiring.tabs.overview"),
        t("hiring.tabs.details"),
        t("hiring.tabs.cost"),
    ]


def _takeover_dimension_label(dimension: str) -> str:
    if dimension == "JobFamily":
        return t("hiring.azubi.takeover_dimension.jobfamily")
...
def main():
    ...
    try:
        # 1. Load Central Data (Consistent with AbgÃƒÆ’Ã‚Â¤nge/Kompakt)
        ...
        # 5. Preprocessing (Align with AbgÃƒÆ’Ã‚Â¤nge/Kompakt)
        # Remove Vacancies
    ...
    with st.expander(t("hiring.settings.expander"), expanded=True):
        with st.form("forecast_inputs"):
            ...
            with tab_azubi:
                ...
                duration = c3.number_input(t("hiring.azubi.duration_years"), 1.0, 5.0, params["azubi"]["duration_years"], 0.5)
                az_strat = c4.selectbox(t("hiring.common.distribution"), ["Random", "OrgUnit"], index=0 if params["azubi"]["strategy"] == "Random" else 1, key="az_strat")
                ...
                c5, c6 = st.columns(2)
                az_tarif = c5.selectbox(t("hiring.azubi.entry_tariff"), TARIFF_GROUPS, index=TARIFF_GROUPS.index(params["azubi"]["entry_tariff_group"]) if params["azubi"]["entry_tariff_group"] in TARIFF_GROUPS else 5, key="az_tarif")
                az_step = c6.number_input(t("hiring.azubi.entry_step"), 1, 6, params["azubi"]["entry_step"], key="az_step")

                # Graduation Mode Selector
                cg1, cg2 = st.columns([1, 2])
                with cg1:
                    az_graduation_mode = st.radio(
                        t("hiring.azubi.graduation_mode"),
                        options=["nearest_cycle", "next_cycle"],
                        index=0 if params["azubi"].get("graduation_mode", "nearest_cycle") == "nearest_cycle" else 1,
                        format_func=lambda x: t("hiring.azubi.graduation_mode.nearest_cycle") if x == "nearest_cycle" else t("hiring.azubi.graduation_mode.next_cycle"),
                        horizontal=True,
                ...
                az_takeover_matrix = render_distribution_matrix(
                    label=t("hiring.azubi.takeover_matrix.label", dimension=az_dim),
                    dimension=az_dim,
                ...
                if st.session_state.get("chk_forecast_debug", False):
                    ...
                    with st.expander("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Debug: UI Matrix Snapshot", expanded=False):
                        st.json(ui_snap)
            ...
            with tab_hires:
                ...
                with st.expander(t("hiring.hires.distribution.heading"), expanded=False):
                    ...
                    if not (0.99 <= total_share <= 1.01):
                        st.warning(f"ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Summe der Anteile ist {total_share:.2%} (sollte 100% sein). Werte werden bei der Simulation normalisiert.")
    ...
    if submit:
        ...
        if st.session_state.get("chk_forecast_debug", False):
            ...
            with st.expander("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â Debug: Diagnose vor Start", expanded=True):
                # Consistency Check
                if ui_snap and sim_snap != ui_snap:
                    st.error("ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â INKONSISTENZ: UI Matrix und Simulation Input weichen ab!")
                    ...
                    st.success("ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Konsistenz-Check: UI Matrix == Simulation Input")
                ...
                with col_dbg1:
                    ...
                    if use_az_matrix:
                        ...
                        st.write(f"- BefÃƒÆ’Ã‚Â¼llte Kategorien: {len(weights)}")
                ...
                if use_az_matrix:
                    st.markdown("**Top 5 Matrix-EintrÃƒÆ’Ã‚Â¤ge:**")
        ...
        # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ 1. Abgangs-Prognose (Basis fÃƒÆ’Ã‚Â¼r Netto-Betrachtung) ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
        # Wir berechnen IMMER die AbgÃƒÆ’Ã‚Â¤nge, um den Netto-Headcount zeigen zu kÃƒÆ’Ã‚Â¶nnen.
        
        with st.spinner("Berechne Basisszenario (AbgÃƒÆ’Ã‚Â¤nge)..."):
            try:
        ...
        # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ 2. Zugangs-Prognose ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
        ...
        # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ 3. Ergebnis-Aufbereitung (Enrichment) ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
        ...
        if st.session_state.get("chk_forecast_debug", False):
            with st.expander("ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  Debug: Diagnose nach Simulation", expanded=True):
    ...
    if res is not None:
        ...
        # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ 3. Ergebnis-Aufbereitung ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
        ...
        if df_filtered_rows.empty:
            st.warning(t("hiring.warning.no_filtered_data"))
        ...
        if events_df.empty and (hire_count > 0 or not vacancies):
            if hire_count > 0: st.info(t("hiring.info.no_visible_entries"))

        # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Ergebnisse ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
        st.divider()
        ...
        if not forecast_kpis.empty:
            ...
            # KPI: Net Change (Headcount Delta) vs Gross Entries ("ZugÃƒÆ’Ã‚Â¤nge")
            ...
            st.markdown(t("hiring.summary.section"))
            m1, m2, m3 = st.columns(3)
            with m1:
                # Show Gross Entries as "ZugÃƒÆ’Ã‚Â¤nge"
                _tooltip = f"Gesamt: {gross_entries}"
                if gross_entries_internal > 0:
                    _tooltip += f" | davon {gross_entries_external} externe Neueinstellungen + {gross_entries_internal} interne ÃƒÆ’Ã…â€œbernahmen (Azubis)"
                st.metric("Gesamt ZugÃƒÆ’Ã‚Â¤nge (KÃƒÆ’Ã‚Â¶pfe)", f"{gross_entries}", help=_tooltip)
            with m2:
                # Show Net Growth as Delta
                st.metric("ÃƒÅ½Ã¢â‚¬Â Personalvolumen (Ende)", f"{last['mak_end'] - first['mak_start']:+.1f} FTE")
            with m3:
                st.metric("ÃƒÅ½Ã¢â‚¬Â Personalkosten (Jahr)", f"{total_added_cost:,.0f} ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬")
            ...
            if use_azubis:
                if use_trainees or use_newhires:
                    ...
                    info_parts.append("Auszubildende erhÃƒÆ’Ã‚Â¶hen zunÃƒÆ’Ã‚Â¤chst nur KÃƒÆ’Ã‚Â¶pfe; MAK wird erst bei der ÃƒÆ’Ã…â€œbernahme wirksam (zeitverzÃƒÆ’Ã‚Â¶gert).")
                else:
                    info_parts.append("Auszubildende erhÃƒÆ’Ã‚Â¶hen zunÃƒÆ’Ã‚Â¤chst nur den Personalbestand (KÃƒÆ’Ã‚Â¶pfe).")
                    info_parts.append("MAK/FTE werden erst bei der ÃƒÆ’Ã…â€œbernahme nach Ausbildungsende wirksam (zeitverzÃƒÆ’Ã‚Â¶gert).")
            elif use_trainees or use_newhires:
                info_parts.append("Trainee- und externe Einstellungen sind i.d.R. sofort MAK/FTE-wirksam und erhÃƒÆ’Ã‚Â¶hen KÃƒÆ’Ã‚Â¶pfe und MAK gleichzeitig.")
            
            if info_parts:
                st.info(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¡ **Hinweis:** {' '.join(info_parts)}")
            
            # DEBUG OUTPUT
            if st.session_state.get("debug_active", False):
                with st.expander("ÃƒÂ°Ã…Â¸Ã‚ÂÃ…Â¾ Debug-Daten (ZugÃƒÆ’Ã‚Â¤nge)", expanded=True):
                    show_technical_debug = st.toggle(
                        "ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â§ Technische Audit-Kennzahlen anzeigen",
                        value=False,
                    ...
                    if show_technical_debug:
                        ...
                        st.caption(
                            "Diese Werte enthalten zusÃƒÆ’Ã‚Â¤tzlich interne Lifecycle- und Hilfs-Events "
                            "zur Validierung der Prognoselogik und entsprechen nicht den tatsÃƒÆ’Ã‚Â¤chlichen PersonalzugÃƒÆ’Ã‚Â¤ngen."
                        )
            ...
            with st.expander("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Details: Bestandsentwicklung (Brutto-Zuwachs)", expanded=False):
            ...
            def _render_debug_metric(label, chart_val, global_val, unit=""):
                ...
                icon = "ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦" if is_ok else "ÃƒÂ¢Ã‚ÂÃ…â€™"
            ...
            with tab_overview:
                ...
                st.markdown("### ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‹â€  Entwicklung Headcount & MAK")
                fig_evo = px.line(
                    forecast_kpis, 
                    x="period_end", 
                ...
                fig_evo.update_traces(name="KÃƒÆ’Ã‚Â¶pfe (Headcount)", selector=dict(name="headcount_end"))
                ...
                def build_entries_caption(use_azubis, use_trainees, use_newhires):
                    base = "Brutto-Entwicklung durch ZugÃƒÆ’Ã‚Â¤nge (ohne BerÃƒÆ’Ã‚Â¼cksichtigung von AbgÃƒÆ’Ã‚Â¤ngen)."
                    if use_azubis:
                        if use_trainees or use_newhires:
                            ext = "(Mix: Trainees/Neueinstellungen sofort MAK-wirksam; Azubis zeitverzÃƒÆ’Ã‚Â¶gert)"
                        else:
                            ext = "(Azubis: MAK-Wirkung erst bei ÃƒÆ’Ã…â€œbernahme)"
                    else:
                        ext = "(ZugÃƒÆ’Ã‚Â¤nge sind sofort MAK-wirksam)"
                    return f"{base} {ext}"
                ...
                st.markdown("### ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â¥ ZugÃƒÆ’Ã‚Â¤nge nach Quelle")
                ...
                # B) Echte ZugÃƒÆ’Ã‚Â¤nge (Inflows): headcount_change > 0 -> Hire, Conversion_In, New_Hire, Trainee_Hire
                ...
                if not events_inflows.empty:
                    # 2. Deutschsprachige Labels fÃƒÆ’Ã‚Â¼r Chart und Legende
                    label_map = {
                        "Azubi_Hire": "Neue Auszubildende",
                        "Azubi_Conversion_In": "ÃƒÆ’Ã…â€œbernahme aus Ausbildung", 
                    ...
                    fig_hist = px.histogram(
                        ...
                        color="Quelle", 
                        text_auto=True,
                        labels={"date": "Datum", "count": "Anzahl (ZugÃƒÆ’Ã‚Â¤nge)", "Quelle": "Quelle"},
                        # Use specific colors for clarity
                        color_discrete_map={
                            "Neue Auszubildende": COLORS.get("accent_blue", "#1f77b4"),
                            "ÃƒÆ’Ã…â€œbernahme aus Ausbildung": "#9467bd", # Purple (Transformation)
                    ...
                    def build_source_caption(use_azubis, use_trainees, use_newhires):
                        if use_azubis:
                            if use_trainees or use_newhires:
                                return "ÃƒÆ’Ã…â€œbernahmen erhÃƒÆ’Ã‚Â¶hen den MAK (interne Stellenbesetzung). Auszubildende zÃƒÆ’Ã‚Â¤hlen wÃƒÆ’Ã‚Â¤hrend der Ausbildung nur als KÃƒÆ’Ã‚Â¶pfe; Trainees und externe Neueinstellungen sind i.d.R. sofort MAK-wirksam."
                            else:
                                return "ÃƒÆ’Ã…â€œbernahmen (Azubi-ÃƒÆ’Ã…â€œbernahme) sind interne Stellenbesetzungen und erhÃƒÆ’Ã‚Â¶hen den MAK. Neue Auszubildende zÃƒÆ’Ã‚Â¤hlen wÃƒÆ’Ã‚Â¤hrend der Ausbildung nur als KÃƒÆ’Ã‚Â¶pfe; MAK wird erst bei ÃƒÆ’Ã…â€œbernahme wirksam."
                        else:
                            if use_trainees and use_newhires:
                                return "Trainee- und externe Neueinstellungen sind i.d.R. sofort MAK-wirksam und erhÃƒÆ’Ã‚Â¶hen KÃƒÆ’Ã‚Â¶pfe und MAK gleichzeitig."
                            elif use_trainees:
                                return "Trainee-Einstellungen sind i.d.R. sofort MAK-wirksam und erhÃƒÆ’Ã‚Â¶hen KÃƒÆ’Ã‚Â¶pfe und MAK gleichzeitig."
                            elif use_newhires:
                                return "Externe Neueinstellungen sind i.d.R. sofort MAK-wirksam und erhÃƒÆ’Ã‚Â¶hen KÃƒÆ’Ã‚Â¶pfe und MAK gleichzeitig."
                            return ""

                    fig_hist.update_layout(title=None, xaxis_title="Datum", yaxis_title="Anzahl (ZugÃƒÆ’Ã‚Â¤nge)")
                    ...
                    if st.session_state.get("debug_active", False):
                        st.markdown("#### ÃƒÂ°Ã…Â¸Ã‚ÂÃ…Â¾ Debug-Analyse (ZugÃƒÆ’Ã‚Â¤nge)")
                        
                        # Treiber-Status Tabelle
                        driver_data = [
                            {
                                "Treiber": "Azubis ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Neueinstellungen", 
                                "Aktiv (UI)": use_azubis, 
                                "Events (technisch)": len(events_external_azubi),
                                "Inflow-Events (gezÃƒÆ’Ã‚Â¤hlt)": len(events_external_azubi)
                            },
                            {
                                "Treiber": "Azubis ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ÃƒÆ’Ã…â€œbernahmen", 
                                "Aktiv (UI)": use_azubis, 
                                "Events (technisch)": len(events_internal) + len(events_status_change_out),
                                "Inflow-Events (gezÃƒÆ’Ã‚Â¤hlt)": len(events_internal)
                            },
                            {
                                "Treiber": "Trainees", 
                                "Aktiv (UI)": use_trainees, 
                                "Events (technisch)": len(events_trainee),
                                "Inflow-Events (gezÃƒÆ’Ã‚Â¤hlt)": len(events_trainee)
                            },
                            {
                                "Treiber": "Neueinstellungen", 
                                "Aktiv (UI)": use_newhires, 
                                "Events (technisch)": len(events_external_hire),
                                "Inflow-Events (gezÃƒÆ’Ã‚Â¤hlt)": len(events_external_hire)
                            },
                        ]
                        ...
                        with col_d1:
                            ...
                            st.write(f"ZugÃƒÆ’Ã‚Â¤nge gesamt (Inflows): `{len(events_inflows)}`")
                            st.write(f"- Neue Auszubildende: `{len(events_external_azubi)}`")
                            st.write(f"- ÃƒÆ’Ã…â€œbernahmen (intern): `{len(events_internal)}`")
                            st.write(f"- Neueinstellungen (extern): `{len(events_external_hire)}`")
                            st.write(f"- Trainee-Einstellungen: `{len(events_trainee)}`")
                            
                            st.divider()
                            st.caption("*(Hinweis: Die ZugÃƒÆ’Ã‚Â¤nge-Seite zÃƒÆ’Ã‚Â¤hlt nur Inflows. Ein 'Conversion_Out' kann als technischer Statuswechsel zur Paarbildung auftreten, wird aber nicht als Abgang bewertet.)*")
                        
                        with col_d2:
                            st.markdown("**Unique Personen (KÃƒÆ’Ã‚Â¶pfe)**")
                            st.write(f"Unique Personen (Inflows): `{unique_inflows}`")
                            st.write(f"- Unique Auszubildende: `{unique_ext_azubi}`")
                            st.write(f"- Unique ÃƒÆ’Ã…â€œbernahmen: `{unique_int_conv}`")
                            ...
                            if sum_events == len(events_inflows):
                                st.success(f"ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Summencheck: {sum_events} = {len(events_inflows)}")
                            else:
                                st.error(f"ÃƒÂ¢Ã‚ÂÃ…â€™ Summencheck: {sum_events} != {len(events_inflows)}")

                        _render_debug_metric("ZugÃƒÆ’Ã‚Â¤nge (Events): Chart vs Inflows", len(events_inflows), len(events_inflows), "")
                else:
                ...
                st.markdown("### ÃƒÂ°Ã…Â¸Ã‚Â§Ã‚Â© ZugÃƒÆ’Ã‚Â¤nge nach OE-Clustern")
                ...
                if is_clustering_active():
                    if "OE-Cluster" in events_inflows.columns:
                        ...
                        # Chart 1: KopfzugÃƒÆ’Ã‚Â¤nge
                        st.markdown("#### ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â¤ ZugÃƒÆ’Ã‚Â¤nge nach Personen (OE)")
                        c_stats_h = events_inflows.groupby("OE-Cluster").size().reindex(all_clusters, fill_value=0).reset_index(name="ZugÃƒÆ’Ã‚Â¤nge")
                        c_stats_h = c_stats_h.sort_values("ZugÃƒÆ’Ã‚Â¤nge", ascending=True)
                        ...
                        fig_h = px.bar(
                            ...
                            orientation="h",
                            title=None,
                            labels={"OE-Cluster": "Bereich (OE)", "ZugÃƒÆ’Ã‚Â¤nge": "Anzahl Personen"},
                            text="ZugÃƒÆ’Ã‚Â¤nge",
                            color="ZugÃƒÆ’Ã‚Â¤nge",
                            color_continuous_scale="Blues"
                        )
                        fig_h.update_layout(yaxis_title=None, showlegend=False, height=600)
                        st.plotly_chart(fig_h, use_container_width=True)
                        # Debug OE Headcount
                        _render_debug_metric("OE Cluster Headcount", c_stats_h["ZugÃƒÆ’Ã‚Â¤nge"].sum(), gross_entries, "")
                        
                        st.divider()

                        # Chart 2: MAK-Zuwachs
                        st.markdown("#### ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  ZugÃƒÆ’Ã‚Â¤nge nach KapazitÃƒÆ’Ã‚Â¤t (MAK) (OE)")
                        if "mak" in events_inflows.columns:
                            ...
                            st.info("MAK-Daten fÃƒÆ’Ã‚Â¼r Cluster nicht verfÃƒÆ’Ã‚Â¼gbar.")
                    else:
                    ...
                    st.markdown("### ÃƒÂ°Ã…Â¸Ã‚Â§Ã‚Â© ZugÃƒÆ’Ã‚Â¤nge nach Job-Family-Clustern")
                    if "JF-Cluster" in events_inflows.columns:
                        ...
                        # Chart 1: KopfzugÃƒÆ’Ã‚Â¤nge JF
                        st.markdown("#### ÃƒÂ°Ã…Â¸Ã¢â‚¬ËœÃ‚Â¤ ZugÃƒÆ’Ã‚Â¤nge nach Personen (JF)")
                        c_stats_h_jf = events_inflows.groupby("JF-Cluster").size().reindex(all_jf_clusters, fill_value=0).reset_index(name="ZugÃƒÆ’Ã‚Â¤nge")
                        c_stats_h_jf = c_stats_h_jf.sort_values("ZugÃƒÆ’Ã‚Â¤nge", ascending=True)
                        ...
                        fig_h_jf = px.bar(
                            ...
                            orientation="h",
                            title=None,
                            labels={"JF-Cluster": "Berufsgruppe (JF)", "ZugÃƒÆ’Ã‚Â¤nge": "Anzahl Personen"},
                            text="ZugÃƒÆ’Ã‚Â¤nge",
                            color="ZugÃƒÆ’Ã‚Â¤nge",
                            color_continuous_scale="Blues"
                        )
                        ...
                        # Debug JF Headcount
                        _render_debug_metric("JF Cluster Headcount", c_stats_h_jf["ZugÃƒÆ’Ã‚Â¤nge"].sum(), gross_entries, "")

                        st.divider()

                        # Chart 2: MAK-Zuwachs JF
                        st.markdown("#### ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…Â  ZugÃƒÆ’Ã‚Â¤nge nach KapazitÃƒÆ’Ã‚Â¤t (MAK) (JF)")
                        if "mak" in events_inflows.columns:
            ...
            with tab_details:
                st.markdown("### ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Detaillierte Liste der ZugÃƒÆ’Ã‚Â¤nge")
                ...
                # for the same person on the same date ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â net HC=0. Showing them in a raw
                ...
                if _has_internal_flag:
                    ...
                    if _n_internal > 0:
                        st.caption(
                            f"ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¹ÃƒÂ¯Ã‚Â¸Ã‚Â {_n_internal} interne Statuswechsel-Zeilen enthalten "
                            f"(Azubi-ÃƒÆ’Ã…â€œbernahme: je 1ÃƒÆ’Ã¢â‚¬â€ Conv_Out + 1ÃƒÆ’Ã¢â‚¬â€ Conv_In pro Person). "
            ...
            with tab_cost:
                st.markdown("### ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â° Kosten-Impact")
                if not events_df.empty:
                    ...
                    fig_cost = px.bar(
                        ...
                        labels={"Month": "Monat", "Cost_Impact": "Kosten-Impact (Jahr) in ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬", "Quelle": "Quelle"}
                    )
                    ...
                    st.info("Keine Kostendaten verfÃƒÆ’Ã‚Â¼gbar (da keine ZugÃƒÆ’Ã‚Â¤nge).")
